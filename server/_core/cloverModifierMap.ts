/**
 * cloverModifierMap.ts
 *
 * Static map of Clover modifier IDs for pizza options.
 * These IDs were created/verified via the Clover API and must not be changed
 * without re-verifying against the merchant account (MRWSQWMCDSHQ1).
 *
 * Modifier Groups:
 *   Pizza Crust → VMHY08WAQHZ5J
 *   Pizza Cut   → X25FH8C1XGABW
 *   Toppings    → ZBQA3B76GD2RY  (1 Toppings Incluid - 30 options, price=0 per topping)
 *
 * Usage: when building Clover line item modifications, look up the modifier ID
 * from these maps and POST to /orders/{id}/line_items/{lineItemId}/modifications
 */
...
// ── Pizza Topping modifier IDs (group ZBQA3B76GD2RY — 1 Toppings Incluid) ─
// Keys are lowercase, trimmed topping names matching PIZZA_30_TOPPINGS in napoliData.ts
export const CLOVER_TOPPING_IDS: Record<string, string> = {
  ...
  "broccoli":             "", // not in Clover — will fall back to note
  "zucchini":             "", // not in Clover — will fall back to note
  "capicola":             "", // not in Clover — will fall back to note
  "cheddar cheese":       "Z5TYARCWHSTK6",
  "grilled chicken":      "FX0XK710WWBTA",
  "chorizo":              "", // not in Clover — will fall back to note
  ...
  "canadian bacon":       "", // not in Clover — will fall back to note
};

// ── Modifier group IDs (for reference) ───────────────────────────────────
export const CLOVER_MODIFIER_GROUP_IDS = {
  PIZZA_CRUST:    "VMHY08WAQHZ5J",
  PIZZA_CUT:      "X25FH8C1XGABW",
  PIZZA_TOPPINGS: "ZBQA3B76GD2RY",
} as const;
