 ...
 * Step 5 — OPTIONAL: choose sauces (multi-select, all free)
...
function BurgerCustomizerInner({
  ...
  // If burger + size are pre-selected, jump straight to doneness (step 3)
  ...
  const stepDone = (n: number) => {
    ...
    if (n === 3) return doneness !== "" && step > 3;
    if (n === 4) return breadChoice !== "" && step > 4;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── STEP INDICATOR ─────────────────────────────────── */}
        <div className="flex items-center px-3 py-2.5 gap-0.5 shrink-0 overflow-x-auto" style={{ background: "oklch(0.97 0.012 80)" }}>
          {STEPS.map((s, i) => {
            ...
            <div className="px-5 py-5">
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  style={burgerName ? { background: "var(--napoli-red)", color: "white" } : { background: "oklch(0.88 0.015 80)", color: "oklch(0.62 0.03 30)", cursor: "not-allowed" }}
                >
          ...
          {step === 2 && (
            <div className="px-5 py-5">
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg mb-4" style={{ background: "oklch(0.97 0.012 80)", border: "1px solid oklch(0.88 0.015 80)" }}>
                ...
                <span className="text-xs napoli-body font-semibold" style={{ color: "var(--napoli-dark)" }}>{burgerName}</span>
                <button onClick={() => setStep(1)} className="ml-auto text-xs napoli-label underline shrink-0" style={{ color: "oklch(0.52 0.03 30)" }}>Change</button>
              </div>

              <p className="napoli-label text-xs mb-1" style={{ color: "var(--napoli-red)", letterSpacing: "0.15em" }}>CHOOSE SIZE <span>*</span></p>
                    ...
                    {(["half", "single"] as const).map((sz) => {
                      ...
                      return (
                        <button
                          ...
                          style={burgerSize === sz ? { background: "var(--napoli-red)", border: "2px solid var(--napoli-red)", color: "white" } : { background: "white", border: "2px solid oklch(0.88 0.015 80)", color: "var(--napoli-dark)" }}
                        >
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  disabled={!burgerSize}
                  className="flex items-center gap-2 px-5 py-3 rounded-lg text-sm font-semibold napoli-label transition-all active:scale-95"
                  style={burgerSize ? { background: "var(--napoli-red)", color: "white" } : { background: "oklch(0.88 0.015 80)", color: "oklch(0.62 0.03 30)", cursor: "not-allowed" }}
                >
                  Next: Doneness <ChevronRight size={15} />
                </button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="px-5 py-5">
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg mb-4" style={{ background: "oklch(0.97 0.012 80)", border: "1px solid oklch(0.88 0.015 80)" }}>
                ...
                <button onClick={() => setStep(2)} className="ml-auto text-xs napoli-label underline shrink-0" style={{ color: "oklch(0.52 0.03 30)" }}>Change</button>
              </div>

              <p className="napoli-label text-xs mb-1" style={{ color: "var(--napoli-red)", letterSpacing: "0.15em" }}>
                HOW WOULD YOU LIKE IT COOKED? <span>*</span>
              </p>
              <p className="text-xs napoli-body mb-4" style={{ color: "oklch(0.52 0.03 30)" }}>
                Select one — required.
              </p>
                ...
                {BURGER_DONENESS_OPTIONS.map((d) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs napoli-body mt-0.5" style={{ color: doneness === d.name ? "oklch(0.90 0.015 80)" : "oklch(0.52 0.03 30)" }}>
                        {d.desc}
                      </p>
                    </div>
                    {'isDefault' in d && d.isDefault && doneness !== d.name && (
                      <span className="text-xs px-1.5 py-0.5 rounded napoli-label shrink-0" style={{ background: "oklch(0.92 0.04 80)", color: "oklch(0.42 0.03 30)" }}>
                        Default
                      </span>
                    )}
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  disabled={!doneness}
                  className="flex items-center gap-2 px-5 py-3 rounded-lg text-sm font-semibold napoli-label transition-all active:scale-95"
                  style={doneness ? { background: "var(--napoli-red)", color: "white" } : { background: "oklch(0.88 0.015 80)", color: "oklch(0.62 0.03 30)", cursor: "not-allowed" }}
                >
          ...
          {step === 4 && (
            <div className="px-5 py-5">
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg mb-4" style={{ background: "oklch(0.97 0.012 80)", border: "1px solid oklch(0.88 0.015 80)" }}>
                <Check size={14} style={{ color: "var(--napoli-green)" }} />
                <span className="text-xs napoli-body font-semibold" style={{ color: "var(--napoli-dark)" }}>
                  {burgerName} · {burgerSize === "half" ? "½ lb" : "1 lb"} · {doneness}
                </span>
                <button onClick={() => setStep(3)} className="ml-auto text-xs napoli-label underline shrink-0" style={{ color: "oklch(0.52 0.03 30)" }}>Change</button>
              </div>

              <p className="napoli-label text-xs mb-1" style={{ color: "var(--napoli-red)", letterSpacing: "0.15em" }}>CHOOSE BREAD <span>*</span></p>
              ...
              <div className="flex flex-col gap-3">
                {BURGER_BREAD_OPTIONS.map((b) => (
                  <button
                    ...
                    className="flex items-center gap-4 px-4 py-4 rounded-xl text-left transition-all active:scale-98"
                    style={breadChoice === b.name ? { background: "var(--napoli-red)", border: "2px solid var(--napoli-red)", color: "white" } : { background: "white", border: "2px solid oklch(0.88 0.015 80)", color: "var(--napoli-dark)" }}
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p className="napoli-body text-sm font-bold">{b.name}</p>
                        {b.badge && (
                          <span className="text-xs px-1.5 py-0.5 rounded font-bold" style={{ background: breadChoice === b.name ? "oklch(0.35 0.18 27)" : "oklch(0.92 0.08 145)", color: breadChoice === b.name ? "white" : "oklch(0.30 0.12 145)" }}>
                            {b.badge}
                          </span>
                        )}
                      </div>
                      {b.name === "Gluten Free Bread" && (
                        <p className="text-xs napoli-body mt-0.5" style={{ color: breadChoice === b.name ? "oklch(0.90 0.015 80)" : "oklch(0.52 0.03 30)" }}>No additional charge</p>
                      )}
                    </div>
          ...
          {step === 5 && (
            <div className="px-5 py-5">
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg mb-4" style={{ background: "oklch(0.97 0.012 80)", border: "1px solid oklch(0.88 0.015 80)" }}>
                <Check size={14} style={{ color: "var(--napoli-green)" }} />
                <span className="text-xs napoli-body font-semibold" style={{ color: "var(--napoli-dark)" }}>
                  {burgerName} · {burgerSize === "half" ? "½ lb" : "1 lb"} · {doneness} · {breadChoice}
                </span>
                <button onClick={() => setStep(4)} className="ml-auto text-xs napoli-label underline shrink-0" style={{ color: "oklch(0.52 0.03 30)" }}>Change</button>
              </div>

              <p className="napoli-label text-xs mb-1" style={{ color: "var(--napoli-red)", letterSpacing: "0.15em" }}>CHOOSE YOUR SAUCES</p>
              <p className="text-xs napoli-body mb-4" style={{ color: "oklch(0.52 0.03 30)" }}>All sauces are free — pick as many as you like.</p>
            ...
            <div className="px-5 py-5">
              {selectedSauces.length > 0 && (
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg mb-4" style={{ background: "oklch(0.97 0.012 80)", border: "1px solid oklch(0.88 0.015 80)" }}>
                  ...
                  <button onClick={() => setStep(5)} className="ml-auto text-xs napoli-label underline shrink-0" style={{ color: "oklch(0.52 0.03 30)" }}>Change</button>
                </div>
              )}
              ...
              <p className="text-xs napoli-body mb-3" style={{ color: "oklch(0.52 0.03 30)" }}>Add Cheese · Add Bacon · Add Avocado — $1.00 each</p>
              ...
              {/* Special notes */}
              <p className="napoli-label text-xs mb-2" style={{ color: "var(--napoli-red)", letterSpacing: "0.15em" }}>📝 SPECIAL NOTES (OPTIONAL)</p>
              <textarea
                value={notes}
